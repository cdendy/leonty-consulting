...
export default function Header() {
  ...
  useEffect(() => {
    ...
    // Initialize nextIndex to null so first hover picks random
    ...
    document.querySelectorAll<HTMLElement>("[data-draw-line]").forEach((container) => {
      ...
      container.addEventListener("mouseenter", () => {
        // Do not restart if still drawing in
        ...
        // Pick random start if first time
        if (nextIndex === null) {
    ...
    <header className="bg-white shadow-md sticky top-0 z-50">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex space-x-8">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`relative inline-block text-gray-800 font-medium transition-colors hover:text-gray-900 ${
        ...
        <div className="hidden md:block">
          <Link
            href="/contact"
            className="ml-8 inline-block bg-blue-800 text-white px-4 py-2 rounded-lg hover:bg-blue-900 transition-colors duration-200"
          >
            Book Now
          </Link>
        </div>
